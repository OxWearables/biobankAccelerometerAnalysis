...
def getActivitySummary(epochFile, nonWearFile, summary,
    ...
    # Calculate imputation values to replace nan PA metric values"""Module to generate overall activity summary from epoch data."""
...
def check_daylight_savings_crossover(e, startTime, endTime, summary, timeZone='Europe/London'):
    ...
    if startTimeZone.dst() != endTimeZone.dst():
        ...
        for t in localTime._utc_transition_times:
            if t>startTime:
                transition = t - pd.DateOffset(hours=1)
                transition = transition + localTime.utcoffset(transition)
                break
